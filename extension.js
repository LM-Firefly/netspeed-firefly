...
function speedToString(amount) {
    ...
    if ( mode == 0 || mode == 1 || mode == 2 || mode == 3 ) amount = amount * 8;
...
function getStyle(isIcon = false) {
  return isIcon
    ? "size-" + String(crStng.fontmode)
    : "forall size-" + String(crStng.fontmode);
}
...
function initNsLabels() {
  ...
  extraLabelInfo +=
    "text-align: " + ["left", "right", "center"][crStng.textAlign] + "; ";
  ...
  usLabel = new St.Label({
    ...
    style:
      extraLabelInfo + (crStng.systemColr ? "" : "color: " + crStng.usColor),
  });
  ...
  dsLabel = new St.Label({
    ...
    style_class: getStyle(),
    style:
      extraLabelInfo + (crStng.systemColr ? "" : "color: " + crStng.dsColor),
  });
  ...
  tsLabel = new St.Label({
    ...
    style_class: getStyle(),
    style:
      extraLabelInfo + (crStng.systemColr ? "" : "color: " + crStng.tsColor),
  });
  ...
  tdLabel = new St.Label({
    ...
    style_class: getStyle(),
    style:
      extraLabelInfo + (crStng.systemColr ? "" : "color: " + crStng.tdColor),
  });
  usIcon = new St.Label({
    text: DIcons(1),
    y_align: Clutter.ActorAlign.CENTER,
...
function initNs() {
  ...
  if (crStng.mode == 0 || crStng.mode == 1) {
    ...
    if (crStng.showTotalDwnld) {
      nsLayout.attach(
        !crStng.iconsToRight ? tdIcon : tdLabel,
        widthConstant,
        heightConstant,
        1,
        1
      ...
      nsLayout.attach(
        !crStng.iconsToRight ? tdLabel : tdIcon,
        1 + widthConstant,
        heightConstant,
        1,
        1
    ...
    if (crStng.revIndicator) {
      ...
      nsLayout.attach(
        !crStng.iconsToRight ? dsIcon : dsLabel,
        widthConstant,
        heightConstant,
        1,
        1
      ...
      nsLayout.attach(
        !crStng.iconsToRight ? dsLabel : dsIcon,
        1 + widthConstant,
        heightConstant,
        1,
        1
      ...
      nsLayout.attach(
        !crStng.iconsToRight ? usIcon : usLabel,
        widthConstant,
        heightConstant,
        1,
        1
      ...
      nsLayout.attach(
        !crStng.iconsToRight ? usLabel : usIcon,
        1 + widthConstant,
        heightConstant,
        1,
        1
...
function mouseEventHandler(widget, event) {
  if (event.get_button() == 3) {
    ...
    else crStng.showTotalDwnld = !crStng.showTotalDwnld; // right click on other modes brings total downloaded sum
