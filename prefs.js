  ...
  buildPrefsWidget: function () {
    ...
    class NssSpinBtn {
      constructor(
        ...
        let boolComp =
          crStng[name] === settings.get_default_value(name).unpack();
    ...
    resetBtn.connect("clicked", () => {
      ...
      for (i in strArray) {
        settings.set_string(
          strArray[i],
          settings.get_default_value(strArray[i]).unpack()
        );
      }
      for (j in intArray) {
        settings.set_int(
          intArray[j],
          settings.get_default_value(intArray[j]).unpack()
        );
      }
      for (k in doubleArray) {
        settings.set_double(
          doubleArray[k],
          settings.get_default_value(doubleArray[k]).unpack()
        );
      }
      for (l in boolArray) {
        settings.set_boolean(
          boolArray[l],
          settings.get_default_value(boolArray[l]).unpack()
        );
      }
      settings.set_boolean("restartextension", true);
      fetchSettings();
    ...
    new NssComboBox(
      ...
      ["Right", "Left", "Center"],
      "Choose where to Place the extension on the Panel"
    );
    ...
    new NssComboBox(
      ...
      hboxWPosExt,
      "Position(Advanced)",
      ["Prefer Right Side", "Prefer Left Side"],
      "Choose further where to Place the extension"
    );
    ...
    new NssSpinBtn(
      ...
      hboxRTime,
      "Refresh Time",
      "Change Refresh time value from anywhere b/w 1 to 10",
      1.0,
      10.0,
      0.1,
      1
    ...
    new NssToggleBtn(
      ...
      "Vertical Align",
      "isvertical",
      "Changing it will toggle Vertical Alignment"
    );
    ...
    new NssToggleBtn(
      ...
      "togglebool",
      "Enabling it will show sigma by default"
    );
    ...
    new NssComboBox(
      ...
      "Choose Icons Set",
      [" ⬇,  ⬆", " ▼,  ▲", " ↓,  ↑"],
      "Choose which icon set to display"
    );
    ...
    new NssComboBox(
      ...
      "Limit Unit",
      ["(None)", "K", "M", "G", "T", "P", "E", "Z"],
      "Choose unit limitation set to display"
    );
    ...
    new NssToggleBtn(
      ...
      "hideindicator",
      "Enabling it will Hide Indicator when disconnected"
    );
    ...
    new NssToggleBtn(
      ...
      "Shorten Units",
      "shortenunits",
      "Enabling it will result in shorten units like K instead of KB"
    );
    ...
    new NssToggleBtn(
      ...
      "Show Upload First",
      "reverseindicators",
      "Enabling it will reverse the upload and download speed indicators"
    );
    ...
    new NssToggleBtn(
      ...
      "Lock Mouse Actions",
      "lockmouseactions",
      "Enabling it will Lock Mouse Actions"
    );
    ...
    new NssSpinBtn(
      ...
      hboxMinWidth,
      "Minimum Width",
      "Change Minimum Width value from anywhere b/w 3em to 10em",
      3.0,
      10.0,
      0.5,
      1
    ...
    new NssEntry(
      ...
      "Custom Font Name",
      "customfont",
      "Enter the font name you want, you can also write style here for all elements except indicators"
    );
    ...
    new NssToggleBtn(
      ...
      "systemcolr",
      "Enabling it will allow changing font color dynamically based on panel color"
    );
    ...
    new NssColorBtn(
      ...
      "Download Speed Color",
      "dscolor",
      "Select the download speed color"
    );
    ...
    new NssColorBtn(
      ...
      "Total Download Color",
      "tdcolor",
      "Select the total download color"
    );
